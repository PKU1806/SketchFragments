...
void traffic_generator(int host_num, int conn_num, int done_num) {
	...
	ofstream ofout;
	ofout.open("log.txt", ios::out | ios::trunc);
	ofout.close();
	...
	while (done_count != done_num){
		...
		if (find(done_index_array.begin(), done_index_array.end(), temp) == done_index_array.end()){
			done_index_array.push_back(temp);
			done_count++;
		}
	}
	...
	for (int i = 0; i < host_num; i++) {
		for (int j = 0; j < host_num; j++) {
			if (j != i && find(done_index_array.begin(), done_index_array.end(), i) == done_index_array.end()) {
				string sname = hname[i];
				string rname = hname[j];

				if (find(done_index_array.begin(), done_index_array.end(), j) == done_index_array.end())
				{
				...
				{
					pair[pair_index++] = new thread(Connector_pair, receiver, sender, sname, rname,
					   	nullptr, std::ref(current_conn_num), conn_num);
				}
	...
	for (int i = 0; i < (host_num - done_num) * (host_num - 1); i++)
	{
		pair[i] -> join();
	}
...
int main(int argc, char **argv) {
	...
	traffic_generator(host_num, conn_num, done_num);
